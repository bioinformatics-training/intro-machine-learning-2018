...
We will use repeated cross-validation to find the best value of *k*. Repeated cross-validation can readily be parallelized to increase speed of execution. All we need to do is create a local cluster - **caret** will then use this cluster to parallelize the cross-validation.
...
for(i in 1:25) seeds[[i]] <- sample.int(1000, length(tuneParam$k))
...
Predict test set and tabulate results in a confusion matrix
```{r}
test_pred <- predict(knnFit, morphologyTest)
confusionMatrix(test_pred, stageTest)
```

Finally we can stop our cluster if we no longer need it
```{r}
stopCluster(cl)
```
